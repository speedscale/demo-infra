...
- Report status should distinguish provisional tracker timeouts from terminal replay failures. `banking-accounts` returned `Error` after the 5-minute tracker timeout, then analyzer completed the same report as `Missed Goals` about 5 minutes later. CI should not have to guess whether `Error` is final while analyzer/replay signals are still progressing.
